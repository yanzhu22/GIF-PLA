...
def inter_graph(ligand, pocket, dis_threshold = 6.):
...
# %%
def mols2graphs(complex_path, label, save_path, dis_threshold=6.0):
    try:
...
class GraphDataset(object):
    ...
    def __init__(self, data_dir, data_df, dis_threshold=6.0, graph_type='Graph_EHIGN', num_process=48, create=True):
    ...
    def _pre_process(self):
        ...
        if self.create:
            ...
            pool.close()
            mols2graphs(complex_path_list, pKa_list, graph_path_list, dis_thresholds)
            pool.join()
    ...
    def __getitem__(self, idx):
        complex_path = os.path.join(self.data_dir, self.complex_ids[idx], f"{self.complex_ids[idx]}-6.rdkit")
        graph_path = os.path.join(self.data_dir, self.complex_ids[idx], f"{self.graph_type}-{self.complex_ids[idx]}-6.dgl")
        if not os.path.exists(graph_path):
            mols2graphs(complex_path, self.data_df.loc[idx, '-logKd/Ki'], graph_path, dis_threshold=self.dis_threshold)
...
if __name__ == '__main__':
    ...
    toy_dir = os.path.join(data_root, 'pdbbind_files')
    toy_df = pd.read_csv(os.path.join(data_root, "./train_add_seq_smiles.csv"))
    toy_set = GraphDataset(toy_dir, toy_df, graph_type='Graph_EHIGN', dis_threshold=6., create=True)
    toy_loader = DataLoader(toy_set, batch_size=32, shuffle=True, collate_fn=collate_fn, num_workers=1)
    
    bg, label = next(iter(toy_loader))
    atom_feats = bg.ndata['h']
    bond_feats = bg.edata['e']

    yi = bg.edges['intra_l'].data['e']
