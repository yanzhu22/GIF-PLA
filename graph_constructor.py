...
def collate_fn(data_batch):
    ...
    mbg = dgl.batch(mg)
    mbg2 = dgl.batch(mg2)
    sequence = torch.from_numpy(np.array(seq))
    smiles = torch.from_numpy(np.array(smi))
    y = torch.cat(label, dim=0)
    return mbg, mbg2, bg, sequence, smiles, y
...
class GraphDataset(object):
    ...
    def __init__(self, data_dir, data_df, dis_threshold=5.0, MAX_SEQ_LEN=1200,  max_smi_len = 90, graph_type='Graph_GIF_PLA', num_process=48, create=True):
    ...
    def _pre_process(self):
        ...
        for i, row in data_df.iterrows():
            ...
            print(cid)
            #print(cid, pKa, seq, smi)
            complex_dir = os.path.join(data_dir, cid)
            graph_path = os.path.join(complex_dir, f"Graph_GIF_PLA-{cid}.dgl")
            #print(graph_path)
    ...
    def __getitem__(self, idx):
        ...
        return mgs, mgs2, bg, seq, smi, label
...
if __name__ == '__main__':
    data_dir = './train_set/'
    data_file = './train.csv'  # 修改为新的数据文件路径
    data = pd.read_csv(data_file)
    train_set = GraphDataset(data_dir, data, graph_type='Graph_GIF_PLA', dis_threshold=5., create=True)
    train_loader = DataLoader(train_set, batch_size=32, shuffle=True, collate_fn=collate_fn, num_workers=1)
